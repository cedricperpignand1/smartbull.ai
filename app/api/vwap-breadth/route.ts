...
export async function POST(req: Request) {
  try {
    ...
    const results = await Promise.all(
      tickers.map(async (symbol) => {
        try {
          ...
          if (!Array.isArray(bars) || !bars.length) { failed.push(symbol); return { ok: false }; }
    ...
    for (const r of results) {
      if (!r.ok) continue;
      if (r.state === "above") above++; else if (r.state === "below") below++; else flat++;
    }
    const total = above + below + flat;            // succeeded
    const ratio = total ? above / total : 0;

    const payload = {
      ok: true,
      // succeeded counts:
      total, above, below, flat, ratio,
      // new diagnostics/denominator info:
      attempted, failed,
      marketOpen: open,
      tickers,
      session: { dateET: yyyyMmDdET(), startISO, endISO },
    };
