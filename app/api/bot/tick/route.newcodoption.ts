...
import {
  submitBracketBuy,
  closePositionMarket,
  replaceTpSlIfBetter,   // we’ll use this to lift/cancel-by-lifting TP and lift SL
  getPosition,
  getBars1m,
  premarketRangeISO,
...
/** Legacy targets for UI/info; hybrid uses TARGET1_PCT below */
const TARGET_PCT = 0.10;   // shown in info/debug
const STOP_PCT = -0.05;    // -5% SL from fill (initial)
const TOP_CANDIDATES = 8;

/** Disable built-in ratchet so it doesn't fight the trailing logic */
...
/* -------------------------- 70/30 HYBRID config -------------------------- */
const TARGET1_PCT = 0.10;            // +10% triggers partial
const PARTIAL1_FRACTION = 0.70;      // sell 70% at +10%

const BREAKEVEN_TRIGGER_PCT = 0.05;  // once +5%, lift SL to breakeven
const TRAIL_STEP_PCT = 0.05;         // 5% step trail on the remaining ~30%
...
/* -------------------------- order helper (UPDATED for hybrid) -------------------------- */
async function placeEntryNow(ticker: string, ref: number, state: any) {
  ...
  // We'll recenter TP/SL as soon as we get the real fill.
  ...
  // 3) Recenter SL from the REAL fill; and move TP way higher so it won't auto-close runner.
  //    We will do our own 70% partial at +10%, and trail the rest with SL only.
  const newSl = Math.round(entry * (1 + STOP_PCT) * 100) / 100;
  const newTpFar = Math.round(entry * 3 * 100) / 100; // 3x entry as a "ceiling" that won't trigger intraday
  try {
    await (replaceTpSlIfBetter as any)({ symbol: ticker, newTp: newTpFar, newSl, force: true });
  } catch { /* best effort */ }

  // 4) Persist REAL entry.
...
async function handle(req: Request) {
  try {
    ...
    pendingTick = (async () => {
      ...
      // Holding loop (live price, partial 70% at +10%, runner w/ 5% step SL trail; no TP on runner)
      if (openPos) {
        ...
        if (pParsed != null) {
          ...
          try {
            ...
            if (day.length >= 2) {
              ...
              /* --- Base partial: sell 70% at +10% --- */
              const tp1 = Math.round(entry * (1 + TARGET1_PCT) * 100) / 100;
              const alreadyTookPartial =
                partialTPMemo[memoKey]?.taken === true && partialTPMemo[memoKey].day === todayYMD;

              if (!alreadyTookPartial && p >= tp1) {
                const sharesNow = Number(openPos.shares);
                const qtySell = Math.max(1, Math.floor(sharesNow * PARTIAL1_FRACTION));
                if (qtySell >= 1 && qtySell < sharesNow) {
                  try {
                    await sellMarket({ symbol: openPos.ticker, qty: qtySell });

                    const realized = qtySell * (p - entry);
                    const cashIn = qtySell * p;

                    await prisma.trade.create({ data: { side: "SELL", ticker: openPos.ticker, price: p, shares: qtySell } });
                    await prisma.position.update({ where: { id: openPos.id }, data: { shares: sharesNow - qtySell } });
                    state = await prisma.botState.update({
                      where: { id: 1 },
                      data: { cash: Number(state!.cash) + cashIn, pnl: Number(state!.pnl) + realized, equity: Number(state!.cash) + cashIn + (sharesNow - qtySell) * p },
                    });
                    ...
                    // Start trailing for remaining ~30%
                    trailHalfMemo[memoKey] = { day: todayYMD, active: true, anchor: p };

                    // Also shove broker TP far away again (in case child TP qty persisted),
                    // while keeping SL as-is (or better) — avoids surprise full exits.
                    try {
                      const newTpFar = Math.round(entry * 3 * 100) / 100;
                      await replaceTpSlIfBetter({ symbol: openPos!.ticker, newTp: newTpFar });
                    } catch {}
                  } catch (e: any) {
                    debug.reasons.push(`partial_70_sell_exception:${e?.message || "unknown"}`);
                  }
                }
              }

              /* --- Runner: 5% step trailing (SL only; no TP on the runner) --- */
              {
                ...
                if (tstate?.active && openPos) {
                  ...
                  // trailing SL one step behind anchor (never goes down)
                  const desiredSl = Math.round(newAnchor * (1 - TRAIL_STEP_PCT) * 100) / 100;
